		...
		try {
			...
			if (resList !=null) {
				...
				while (resIterator.hasNext()) {
					...
					GSuiteDriveLog gSuiteDriveLog=convertToPoJo(activity);
...
private GSuiteDriveLog convertToPoJo(Activity activity) {
		...
		if (activity.getEvents() !=null) {
			...
			if (eventsList!=null) {
				Events events=eventsList.get(0); 		// Get the first one
